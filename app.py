    ...
    if request.method == 'POST':
                try:
                    ...
                    for index,image_tag in enumerate(image_tags):
                                ...
                                mydict={"Index":f"{query}_{index}","Image":image_data}
                                img_data.append(mydict)
                                with open(os.path.join(save_directory, f"{query}_{image_tags.index(image_tag)}.jpg"), "wb") as f:
                                    f.write(image_data)
                    client = pymongo.MongoClient("mongodb+srv://"+mongo_username+":"+mongo_password+"@cluster0.j3wnbpo.mongodb.net/?retryWrites=true&w=majority")
